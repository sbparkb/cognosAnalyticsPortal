...
public class CognosControl {
	...
	/**
	 * 현재 위치 하위의 컨텐츠 목록을 반환한다.
	 * @param parentPath	상위폴더 경로
	 * @return ArrayList
	 */
	public ArrayList<CognosObject> getSubContentFolderListPath(String parentPath, ContentManagerService_PortType cmService) {
		ArrayList<CognosObject> list = new ArrayList<CognosObject>();
		try {
			PropEnum[] prop = { PropEnum.searchPath, PropEnum.defaultName, PropEnum.objectClass, PropEnum.storeID, PropEnum.target };
			Sort[] sort = { new Sort(), new Sort() };
			sort[0].setPropName(PropEnum.displaySequence);
			sort[0].setOrder(OrderEnum.descending);
			sort[1].setPropName(PropEnum.defaultName);
			sort[1].setOrder(OrderEnum.ascending);
			BaseClass[] bc = cmService.query(new SearchPathMultipleObject(parentPath + "/*[permission('execute')]"), prop, sort, new QueryOptions());			
			if(bc != null && bc.length > 0) {				
				for(int i=0;i<bc.length;i++) {
					if("package".equals(bc[i].getObjectClass().getValue().getValue())) {
						continue;
					} else {
						CognosObject obj = new CognosObject();
						obj.setSearchPath(bc[i].getSearchPath().getValue());
						obj.setDefaultName(bc[i].getDefaultName().getValue());
						obj.setObjectType(bc[i].getObjectClass().getValue().getValue());
						obj.setStoreId(bc[i].getStoreID().getValue().get_value());
						
						if( "shortcut".equals(obj.getObjectType())){
							String path = ((Shortcut)bc[i]).getTarget().getValue()[0].getSearchPath().getValue();
							obj.setTargetSearchPath(path);								
							obj.setTargetObjectType(getCognosObjectSimpleInfo(path, cmService).getObjectType());
						}else if("report".equals(obj.getObjectType())   || "reportView".equals(obj.getObjectType()) || "query".equals(obj.getObjectType()) || "pagelet".equals(obj.getObjectType())
							|| "analysis".equals(obj.getObjectType()) || "URL".equals(obj.getObjectType()) || "folder".equals(obj.getObjectType())) {						
						}
						list.add(obj);
	...
	/**
	 * Cognos Object defaultName, StoreId, ObjectType 조회 
	 * @param searchPath 검색경로
	 * @return
	 */
	public CognosObject getCognosObjectSimpleInfo(String searchPath, ContentManagerService_PortType cmService) {
		
		CognosObject obj = null; 
		try {
			PropEnum[] prop = { PropEnum.searchPath, PropEnum.defaultName, PropEnum.objectClass, PropEnum.storeID };
			BaseClass[] bc = cmService.query(new SearchPathMultipleObject(searchPath), prop, new Sort[] {}, new QueryOptions());
			if(bc != null && bc.length > 0) {
				obj = new CognosObject(); 
				obj.setSearchPath(bc[0].getSearchPath().getValue());
				obj.setDefaultName(bc[0].getDefaultName().getValue());
