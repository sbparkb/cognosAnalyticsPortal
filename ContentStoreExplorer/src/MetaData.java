...
public class MetaData
{
	...
	/**
	 * ParseMetaData method will create a vector of MetaData objects by parsing (using DOM)  the 
	 *   metadata XML returned from the content store. Basically a list of all tables and the columns.
	 * 
	 *   querySubject = tables 
	 *   queryItems = columns
	 * 
	...
	{
		// retrieve the metadata from the selected package to display for table/column selection.
				...
				Element eCurrent =
					(Element)doc.selectSingleNode(
						"/ResponseRoot/folder/folder/folder/querySubject[@name='"
							+ sTable
							+ "']");
				// retrieve a list of columns in this table or queryItems from the current querySubject node.
