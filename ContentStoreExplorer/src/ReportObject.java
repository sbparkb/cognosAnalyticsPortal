...
public class ReportObject
{
	...
	/**
		* This function returns a boolean that reflect the fact that this user's session is
		* ready to interface with the servers.
		*/
	...
	public void modifyColumnTitles(
		Vector columnTitleToChange,
		Vector columnTitleNewName)
	{
		...
		{
			modifyColumnTitle(
				(String)columnTitleToChange.elementAt(k),
	...
	public void modifyColumns(
		...
		Vector originalColumns,
		Vector originalColumnTitle,
		...
		Vector columnsTitle)
	{
		...
		{
			String sOriginalColumnReference =
				(String)originalColumns.elementAt(i);
			String sOriginalColumnExpression =
				(String)originalColumnExpression.elementAt(i);
			String sOriginalColumnTitle =
				(String)originalColumnTitle.elementAt(i);

			boolean columnFound = false;
			for (int j = 0; j < columnExpression.size(); j++)
			{
				if (sOriginalColumnExpression
					.equals(columnExpression.elementAt(j)))
				{
					columnFound = true;
					break;
				}
			...
			{
				System.out.println(
					"remove the column \"" + sOriginalColumnExpression + "\"");
				removeColumn(
					sOriginalColumnExpression,
					sOriginalColumnReference,
					sOriginalColumnTitle);
			}
		}
		...
		for (int i = 0; i < columnExpression.size(); i++)
		{
			...
			{
				if (sColumnExpression
					.equals(originalColumnExpression.elementAt(j)))
				{
					columnFound = true;
					break;
				}
	...
	public Document getMetadata(CRNConnect connection, String p_sModel)
	{
		...
		try
		{
			...
			runResponse = connection.getReportService().runSpecification(metadataSpec, new ParameterValue[] {}, options);
	...
	public String renderReport(CRNConnect connection, String reportSearchPath)
	{
		...
		try
		{
			SearchPathSingleObject reportPath = 
				new SearchPathSingleObject(reportSearchPath); 
			rsr = connection.getReportService().run(
					reportPath,
					new ParameterValue[] {},
			       ...
			       && (rsr.getStatus() != AsynchReplyStatusEnum.conversationComplete) )
			{
				...
				rsr = connection.getReportService().wait(rsq, new ParameterValue[] {}, new Option[] {});
			}
				
			AsynchDetailReportOutput reportOutput = null;
			for (int i = 0; i < rsr.getDetails().length; i++)
			{
				if ( AsynchDetailReportOutput.class.isInstance(rsr.getDetails()[i]) )
				{
	...
	public boolean saveReport(CRNConnect connection, BaseClass parent, String reportName)
	{
	...
	public boolean updateReport(CRNConnect connection, String parentSearchPath, String p_sName)
	{
		...
		try
		{
			connection.getReportService().add(new SearchPathSingleObject(parentSearchPath), rpt, opt);
		}
		catch (Exception e)
		{
		...
		try
		{
			...
			for (int i = 0; i < oBase.length; i++)
			{
					...
					.equals("com.cognos.developer.schemas.bibus._3._package"))
				{
					...
					String oBaseSearchPath =
						oBase[i].getSearchPath().getValue();
