...
public class ReportWizardDialog extends JDialog
{
	...
	public ReportWizardDialog(
		...
		Vector fullNameColumns,
		Vector columnTitles,
		Vector columnTitleExisting,
		Vector columnTitleNew,
		boolean bShowModifyTitleButton)
	{
	 ...
	 * 
	 * @param aContainer 
	 *        Specifies a container for the components, such as the 
	...
	private void buildDialogWindow(Container aContainer)
	{
		...
		if (m_bShowModifyTitleButton)
		{
			...
			modifyColumnTitleButton.addActionListener(
				new modifyColumnTitleButtonListener());
	...
	/**
	 * This Java class is an event handler that runs when a user selects a table. 
	 * It adds a column to the array of available columns for each column in   
	 * the table.
	 **/
	private class tablesComboBoxListener implements ActionListener
	{
		public void actionPerformed(ActionEvent e)
		{
			...
			for (int i = 0; i < m_MetaData.size(); i++)
			{
				...
				{
					availableColumnsList.setListData(
						((MetaData)m_MetaData.elementAt(i)).getQueryItem());
				}
			}
		}
	}
	...
	private class addButtonListener implements ActionListener
	{
		public void actionPerformed(ActionEvent e)
		{
			String selectedColumn =
				(String)availableColumnsList.getSelectedValue();
			String selectedTable = (String)tablesComboBox.getSelectedItem();
			...
			for (int i = 0; i < m_MetaData.size(); i++)
			{
				...
				if (selectedTable.equals(findTable))
				{
					...
					for (int j = 0; j < columnsList.size(); j++)
					{
						...
						{
							MetaData m_MetaDataElement =
								(MetaData)m_MetaData.elementAt(i);
							String columnFullName =
								(String)m_MetaDataElement
									.getFullNameQueryItems()
									.elementAt(
									j);
	 ...
	 * Delete button. It removes the full names of the selected columns 
	 * from the array of columns used to create the report.
	 **/
	private class deleteButtonListener implements ActionListener
	{
		...
		{
			String selectedColumn =
				(String)selectedColumnsList.getSelectedValue();
	...
	/**
	 * This Java class is an event handler that runs when a user clicks the 
	 * OK button. The event handler calls a Java method that releases the 
	 * resources for the dialog box.
	 **/
	 ...
	 * enter a new title for the column, and then removed the old title from 
	 * the array of column titles and adds the new title.
	 **/
	...
	/**
	 * This Java class is an event handler that runs when a user clicks the Cancel button. 
	 * The event handler calls a Java method that cancels the current action.
	 **/
	...
	/**
	 * This Java method releases all screen resources used by this 
	 * dialog box, its subcomponents, and all of its children.
	 **/
	...
	{
		//restore the orginal columns to clear any changes that might have been made
