...
public class SecurityUI extends JFrame
{
	...
	public SecurityUI(String title, CRNConnect connection)
	{
	...
	private void addComponents()
	{
		...
		String cmdList[] = { "Get Logon Info", "Logoff" };
		cboOption = new JComboBox(cmdList);
		buttonPanel.add(cboOption, BorderLayout.WEST);
			...
			try
			{
				...
				if (menuClicked.getText().compareTo("Overview") == 0)
				{
					JFrame explainWindow =
						new JFrame("Overview for Security Sample");
					File explainFile = new File("Java_SecurityUI_Explain.html");
	...
	private class allButtonsHandler implements ActionListener
	{
		public void actionPerformed(ActionEvent e)
		{
			...
			if (buttonPressed == cmdOption)
			{
				...
				if (cboOptionText == "Get Logon Info")
				{
					output = sessionLogon.logonInfo(connect);
				}
				else if (cboOptionText == "Logoff")
				{
					output = sessionLogon.logoff(connect);
				}
				else
				{
					//error
	...
	public static void main(String args[])
	{
		...
		SecurityUI frame = new SecurityUI("IBM Cognos Sample", connection);
