...
public class Capabilities
{
	 ...
	 * @param        securityObject    Account, Group, or Role to be granted access
	 * @param        targetPackage     package to be updated.
	 * @param        secFuncPath       capability to grant access to, as search path.
	 * @param        capToUpdate       capability to grant access to, as Enumeration set value.
	 * 
	 */
	public String updateSecuredFunction(CRNConnect connection, BaseClassWrapper securityObject, BaseClassWrapper targetPackage, String secFuncPath, UserCapabilityEnum capToUpdate)
	{
		
		//add account, group, or role to capability at global level
		if (!(addPolicyToCapability(connection, securityObject, secFuncPath))){
			return "unable to grant selected account or role global " + capToUpdate + " capability";		
		}
		
		//add a policy to package for the specified security object and capability,
		if (!(addPolicyToPackage(connection, securityObject, targetPackage, UserCapabilityEnum.canUseReportStudio))){
			return "unable to grant " + securityObject + " " + capToUpdate + " capability on package " + targetPackage + ".";		
		}
		
		// Both succeeded 
		return "Added " + capToUpdate + " to " + securityObject + " for package " + targetPackage + ".";
	 ...
	 * @param        connection        Connection to Server.
	 * @param        securityObject    Account, Group, or Role to be granted access
	 * @param        capability        Capability to be updated (search path)
	 * 
	 */
	public boolean addPolicyToCapability(CRNConnect connection, BaseClassWrapper securityObject, String capability){
		...
		for (int i = 0; i < numPolicies; i++)
		{
			...
			if (polSecPath.equals(securityObject.getBaseClassObject().getSearchPath().getValue()))
			{
	 ...
	 *
	 * @param        connection        Connection to Server.
	 * @param        targetSecObject   Account, Group, or Role to be granted access
	 * @param        targetPackage     Package to be updated
	 * @param        capToAdd          Capability to be updated (enumeration)
	 * 
	 */
	public boolean addPolicyToPackage(CRNConnect connection, BaseClassWrapper targetSecObject, BaseClassWrapper targetPackage, UserCapabilityEnum capToAdd){
		boolean result = false;
		BaseClass results[] = new BaseClass[] {};
		...
		UserCapabilityPolicy newUCPol = newUCPolicy(targetSecObject, capToAdd, AccessEnum.grant);

		// Create two new arrays of UserCapabilityPolicies - one same size, one plus one
		// If there is already a policy for the account, group, or role is already, 
		// use the first, and include an updated policy for that security object.
		...
		for (int i = 0; i < numPolicies; i++)
		{
			...
			if (polSecPath.equals(targetSecObject.getBaseClassObject().getSearchPath().getValue()))
			{
				tmpPoliciesSame[i] = updateUCPermissions(pol, capToAdd, AccessEnum.grant);
				matchFound = true;
			}
			else{
				tmpPoliciesSame[i] = pol;
	 ...
	 *
	 * @param        secObject		   Account, Group, or Role
	 * @param        capToSet     	   Capability (as enumeration)
	 * @param        grantOrDeny       Access setting to apply
	 * 
	 */
	public UserCapabilityPolicy newUCPolicy(BaseClassWrapper secObject, UserCapabilityEnum capToSet, AccessEnum grantOrDeny){
	 ...
	 *
	 * @param        UserCapabilityPolicy	policy to update
	 * @param        capToSet     	   		Capability (as enumeration)
	 * @param        grantOrDeny       		Access setting to apply
	 * 
	 */
	public UserCapabilityPolicy updateUCPermissions(UserCapabilityPolicy pol, UserCapabilityEnum capToSet, AccessEnum grantOrDeny){
		...
		// Go through permissions array. If setting for capability exists, replace it.
		// Otherwise, add
	 ...
	 * If a capability is present, the current user has this capability at both the
	 * global and package levels. 
	 *
	 * @param        connection        		Connection to Server.
	 * @param        targetPackage			package to check (search path)
	 * @param        capToCheck     	   	Capability (as enumeration)
	 * 
	 */
	public boolean checkEffectiveCapability(CRNConnect connection, String targetPackage, UserCapabilityEnum capToCheck){
		boolean result = false;

		try
		{
			if ((targetPackage == null) || (targetPackage.length() == 0) || (targetPackage.compareTo("") == 0) )
			{
				return false;
			}
			...
			PropEnum[] properties =
				{ PropEnum.defaultName, PropEnum.searchPath, PropEnum.effectiveUserCapabilities};
			...
			try
			{
				...
				result = hasEffectiveCapability(mypackage.getEffectiveUserCapabilities(), capToCheck);
	 ...
	 *
	 * @param        setCapabilities	Array of user capabilities to check
	 * @param        capToSet     	   		Capability (as enumeration)
	 * 
	 */
	public boolean hasEffectiveCapability(UserCapabilityEnumArrayProp setCapabilities, UserCapabilityEnum capToCheck){
