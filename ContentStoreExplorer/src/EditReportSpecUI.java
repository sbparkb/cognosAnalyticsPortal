...
public class EditReportSpecUI extends JFrame
{
			...
			try
			{
				...
				if (menuClicked.getText().compareTo("Overview") == 0)
				{
					JFrame explainWindow =
						new JFrame("Overview for Edit Report Specification Sample");
					File explainFile = new File("Java_EditReportSpecUI_Explain.html");
		...
		public void actionPerformed(ActionEvent e)
		{
			...
			if (buttonPressed == editReportSpecButton)
			{
				try
				{
					...
					String reportSpec =
						editReport.getReportSpec(connect, selectedReport);
	...
	protected BaseClassWrapper[] getListOfReports(CRNConnect connection)
	{
		...
		if ((reports != null) && (reports.length > 0))
		{
			...
			{
				reportList[reportListIndex++] = new BaseClassWrapper(reports[reportIndex]);
			}
		}
		return reportList;
	}
	...
	/** 
		* This java method is used to display a simple dialog box for 
		* removing column and adding columns to a simple list report. 
		* There is also a button to modify a column title.
		* 
		* @param connection   Connection to Server.
		* @param report       Specifies a report object.
		* @param reportSpec   Specifies a report specification XML document.
		*/
	public String editSpec(CRNConnect connection, BaseClassWrapper oldReport, String reportSpec)
	{
		String output = "";
		...
		ReportObject newReport =
			new ReportObject(connect, packageName, reportSpec);
		...
		Vector originalColumnExpression =
			(Vector)selectedColumnExpression.clone();
		Vector originalColumnReference = (Vector)selectedDisplayColumns.clone();
		...
		Vector packageMetaData = md.parseMetaData(connection, newReport, packageName);
		...
		newReport.modifyColumns(
			originalColumnExpression,
			originalColumnReference,
		...
		newReport.saveReport(connection, oldReport.getBaseClassObject().getParent().getValue()[0], oldReport.toString());
		newReport.renderReport(connection, oldReport.getBaseClassObject().getSearchPath().getValue());
		output = "Report: " + oldReport + " updated successfully";
		return output;
	}
	
	
	public String getPackageSearchPath(CRNConnect connection, BaseClassWrapper report){
		
		String reportSearchPath = report.getSearchPath();
		String packagePath = "";
		
		try{
			AsynchReply response = connection.getReportService().query(new SearchPathSingleObject(reportSearchPath), new ParameterValue[] {}, new Option[] {});
			...
			for (int i = 0; i < response.getDetails().length; i++)
			{
				...
				{
					packagePath = ((AsynchDetailReportObject)response.getDetails()[i]).getReport().getMetadataModel().getValue()[0].getSearchPath().getValue();
				}
			}
		...
		{
			System.out.println(
				"An error occurred in the getPackageSearchPath Java method.\n" + e);
...
//		return packagePath;
	...
	public static void main(String args[])
	{
		...
		EditReportSpecUI frame = new EditReportSpecUI("IBM Cognos Sample", connection);
