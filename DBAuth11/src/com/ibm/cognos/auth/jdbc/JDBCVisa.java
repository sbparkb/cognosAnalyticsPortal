...
public class JDBCVisa extends Visa {
	...
	public void init(final JDBCAuth theNamespace, final ConnectionManager theConnectionManager, final String theUsername, final String thePassword) throws UnrecoverableException, UnsupportedEncodingException {
		try {
			...
			DBAuthLogger.debug("--before createAccount--");
			final Account account = QueryUtil.createAccount(connectionManager, theUsername, thePassword);
			DBAuthLogger.debug("--after createAccount--");
			super.init(account);
			DBAuthLogger.debug("--before updateMemberShip--");
			QueryUtil.updateMembership(connectionManager, this);
			DBAuthLogger.debug("--after updateMemberShip--");
